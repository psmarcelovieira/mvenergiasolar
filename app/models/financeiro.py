...
class Financeiro(Base):
    ...
    lancamento_mestre_id = Column(Integer, ForeignKey("lancamentos_mestre.id"), nullable=True)
    parcela_num          = Column(Integer, nullable=True)
    parcela_total        = Column(Integer, nullable=True)
    observacoes          = Column(Text)

    venda             = relationship("Venda")
    lancamento_mestre = relationship("LancamentoMestre", back_populates="parcelas")
